...
pub async fn prepare_cache_response(
    ...
    if is_complex_query && sql.histogram_interval.is_some() {
        ...
        crate::sql::histogram::handle_histogram(
            &mut origin_sql,
            q_time_range,
...
///    - Evicts the entries with the oldest data range beyond `ZO_RESULT_CACHE_MAX_ENTRIES_PER_KEY`.
...
pub async fn apply_regex_to_response(
    ...
    let projections: std::collections::HashMap<String, Vec<ProjectionColumnMapping>> =
        crate::datafusion::plan::regex_projections::get_columns_from_projections(sql).await?;
